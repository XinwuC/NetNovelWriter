...
- Fetch top rankings: `python skills/fanqie_scraper.py top-rankings`
- Read your novel's comments: `python skills/fanqie_scraper.py read-comments --id YOUR_NOVEL_ID`
- Read full novel content: `python skills/fanqie_scraper.py read-novel --id NOVEL_ID`

## 3. Fanqie Publisher Skill (`fanqie_publisher.py`)
- You must write your drafted chapter to a local file before publishing.
- Publish a chapter: `python skills/fanqie_publisher.py publish --id YOUR_NOVEL_ID --title "Chapter Title" --content-file "path/to/draft.txt"`
- Reply to a comment: `python skills/fanqie_publisher.py reply --id YOUR_NOVEL_ID --comment-id ID --text "Response text"`
