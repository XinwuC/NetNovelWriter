...
`openclaw agent --agent {{agent_name}}_planner --session-id "P[X]-S[Y]-$(date +%s)" --message "Run P[X]-S[Y]"`

When Phase 1 and 2 are fully complete, wake up the Coordinator:
`openclaw agent --agent {{agent_name}} --session-id "COORD-$(date +%s)" --message "Novel Planning complete."`

When executing P3-S1 (Chapter Brief), wake up the Writer exactly like this:
`openclaw agent --agent {{agent_name}}_writer --session-id "P3-S2-$(date +%s)" --message "Run P3-S2: Draft Chapter X. Brief: [copy-paste brief here]"`
